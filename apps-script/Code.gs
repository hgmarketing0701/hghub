/**
 * Black Lee - Inventory & Purchasing (Cloud v2.0c-fix3)
 *
 * Features:
 *   - Materials: photoUrl
 *   - Purchases: invoiceUrl, discount, delivery, tax
 *   - Quotations: screenshotUrl
 *   - Grand total = subtotal - discount + delivery + tax
 *
 * v2.0c-fix3 adds auto-migration of legacy column headers left over from
 * the failed v2.1 deployment:
 *   photoFileId      -> photoUrl
 *   invoiceFileId    -> invoiceUrl
 *   screenshotFileId -> screenshotUrl
 *
 * NO Drive scope. Photo URLs are stored as text; the UI auto-converts
 * Drive share URLs to thumbnails.
 *
...
const HEADERS = {
  ...
  Purchases:      ['id','date','supplierId','doNumber','notes','createdAt','createdBy','invoiceUrl','discount','delivery','tax','roundingAdjustment'],
  PurchaseLines:  ['id','purchaseId','materialId','qty','rate','amount','division','requestedBy','itemType'],
  StockOuts:      ['id','dnNumber','date','division','project','notes','createdAt','createdBy','requestedBy'],
...
// Auto-rename legacy column headers left from the failed v2.1 (Drive) deployment.
// Keys are sheet names; each map is { oldHeader: newHeader }.
...
function doGet(e) {
  ...
  return HtmlService.createTemplateFromFile('Index')
    .evaluate()
    .setTitle('Black Lee — Inventory, Tools/Equipment & Purchasing')
...
function renameLegacyColumns_() {
  ...
  Object.keys(LEGACY_COLUMN_RENAMES).forEach(sheetName => {
    ...
    headers.forEach((h, i) => {
      ...
      if (!newName) return;
      // Skip if the new column header already exists in the sheet (avoid duplicates)
...
function ensureSheets_() {
  const ss = ss_();
  // Step 1: rename any legacy column headers from prior deployments
  renameLegacyColumns_();
  // Step 2: create missing sheets and append missing columns
...
function buildPurchaseRecords_() {
  ...
  const byId = {};
  headers.forEach(p => { p.lines = []; byId[p.id] = p; });
  lines.forEach(l => {
...
function buildStockOutRecords_() {
  ...
  const byId = {};
  headers.forEach(o => { o.lines = []; byId[o.id] = o; });
  lines.forEach(l => {
...
/* ===== Tool Assignments (assign + return workflow) ===== */
...
function returnAssignments(payload) {
  return safeCall_('returnAssignments', email => {
    ...
    lines.forEach(line => {
      ...
      if (!cur) return;
      if (cur.returnedDate) return; // already closed
...
/* ===== Category management (rename / delete in bulk) ===== */
  ...
  return safeCall_('savePurchase', email => {
    ...
    try {
      ...
      lines.forEach(l => {
        ...
        // For tool purchases, auto-bump the tool's totalQty
        if (itemType === 'tool') updateToolQty_(l.materialId, qty);
      });
      logAudit_('CREATE', 'Purchase', purchaseId,
        (payload.doNumber || '') + ' · ' + lines.length + ' item(s)' + (payload.invoiceUrl ? ' · invoice link attached' : ''));
...
function deletePurchase(id) {
  return safeCall_('deletePurchase', email => {
    ...
    lines.forEach(l => {
      // Reverse tool stock if this was a tool purchase
